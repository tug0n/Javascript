...
function saveRecord(floorReached, finalScore) {
    ...
    records.push({ floor: floorReached, score: finalScore, date: dateStr });
    records.sort((a, b) => b.score - a.score || b.floor - a.floor);
...
function initGame() {
    ...
    ownedBlessings = []; unlockedFoods = ['gold_chest'];
    isInvincible = false; invincibleTime = 0; foodSpawnCount = 1;
    dangerSensorRange = 2.5 * GRID_SIZE; bulletsCount = 0; maxBulletsPerFloor = 0; cameraScale = 1.0; ownsCrystalHeart = false;
    bullets = []; foods = [];
...
function startNextFloor() {
    ...
    generateBorders(); generateRocks(); generateMonsters();
    foods = []; generateFood();
}
...
function updateMonsters() {
    monsters.forEach(m => {
        ...
        if (dist < 26 && !isInvincible) endGame();
    });
}
...
function update() {
    ...
    if (floor % 5 === 0 && bossSnake.length > 0) {
...
function showBlessingSelection() {
    const blessings = [
        ...
        { id: "rock_breaker", emoji: "☄️", name: "Hỏa Cầu Phá Đá", desc: "Nhận 1 hỏa cầu, bấm Space để bắn phá đá cản đường." },
...
function draw() {
    ...
    monsters.forEach(m => {
        ...
        if (m.type === 'bat') {
            ...
            // Cánh trái
            ctx.moveTo(cx, cy);
            ctx.quadraticCurveTo(cx - 10, cy - 12 + flap * 6, cx - 20, cy - 4 + flap * 4);
            ctx.lineTo(cx - 12, cy + 4);
            ctx.quadraticCurveTo(cx - 6, cy + 2, cx, cy);
            ...
            // Chân bên trái
            ctx.moveTo(cx, cy);
            ctx.lineTo(cx - 12, cy - 12 + wiggle * 3);
            ctx.lineTo(cx - 18, cy - 6 + wiggle * 3);

            ctx.moveTo(cx, cy);
            ctx.lineTo(cx - 14, cy - 4 + wiggle * 2);
            ctx.lineTo(cx - 20, cy + 2 + wiggle * 2);

            ctx.moveTo(cx, cy);
            ctx.lineTo(cx - 14, cy + 4 - wiggle * 2);
            ctx.lineTo(cx - 20, cy + 10 - wiggle * 2);

            ctx.moveTo(cx, cy);
            ctx.lineTo(cx - 12, cy + 12 - wiggle * 3);
            ctx.lineTo(cx - 16, cy + 18 - wiggle * 3);
            ...
            ctx.fillRect(cx - 2, cy - 10, 1.5, 1.5);
            ctx.fillRect(cx + 1, cy - 10, 1.5, 1.5);
    ...
    if (!isDead && !isSelectingBlessing && (direction.x !== 0 || direction.y !== 0)) {
        ...
        // 2. Mũi tên cảnh báo va chạm chướng ngại vật (Màu đỏ)
        if (dangerSensorRange > 0) {
            let closestWall = null, minDistance = Infinity;
                ...
                for (let dy = -checkRadius; dy <= checkRadius; dy++) {
                    ...
                    if (borderWalls.has(key) || rockWalls.has(key)) {
                        ...
                        if (dist < minDistance && dist <= dangerSensorRange) {
                            minDistance = dist;
                            closestWall = { x: wx, y: wy };
                        }
                    }
                }
            }
            if (closestWall) {
                const angle = Math.atan2(closestWall.y - headY, closestWall.x - headX);
...
document.getElementById("records-btn").addEventListener("click", () => { renderRecordsTable(); });
